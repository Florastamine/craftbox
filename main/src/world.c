...
void got_shot()
{
	if (you.skill31 != 2) {return;} // didn't collide with one of player's bullets? Then nothing should happen
...
action player1()
{
	...
	while(my)
	{
		while(PLAYTESTING /* && my.CameraFP && !my.CameraBike */)
		{
			DEBUG_VAR(my.animate,100);
			...
			while(my._HEALTH > 0) {
				...
				if(my.animate >= 100) {
					...
					if(snd_playing(_footstep)) snd_stop(_footstep);
					_footstep = snd_play(footstep,VOL_EFFECTS,0);
				...
				if(sMove == 1) {
					speed.x = ( (key_w || key_cuu) - (key_s || key_cud) ) * 12 * time_step;//6
					speed.y = ( (key_a || key_cul) - (key_d || key_cur) ) * 5 * time_step;
				...
				if(sMove == 2){
					speed.x = ( (key_w || key_cuu) - (key_s || key_cud) ) * 35 * time_step;//11
					speed.y = ( (key_a || key_cul) - (key_d || key_cur) ) * 15 * time_step;
				...
				if( (key_w||key_cuu) && sMove == 1 && my.state != walk) my.blendframe = walk;
				if( (key_w||key_cuu) && sMove == 2 && my.state != run) my.blendframe = run;
				if( (key_s||key_cud) && my.state != walkBack) my.blendframe = walkBack; speed.x += 2 * time_step;
				if( (!key_w||!key_cuu) && (!key_s||!key_cud) && (!key_a||!key_cul) && (!key_d||!key_cur) && my.state != stand) my.blendframe = stand;
