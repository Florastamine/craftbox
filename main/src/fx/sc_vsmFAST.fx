...
*/
	 output.color = 1;
	 float4 lightDepth = texCUBE(cubeShadowMapSampler, -float4(input.LightDir.xyz, 1.0f)+float4(0,0,0,0));
	 
	 /*
    lightDepth.g = lightDepth.g*lightDepth.g;
    //lightDepth.r /= 2;
    float lightDist = (dist_to_light / light_atten_end) - light_shadow_bias*10;
    lightDist *= lightDist;
    lightDist *= lightDist;
    
    if(lightDepth.g < lightDist) output.color.r = 0;
    else output.color.r = 1;
    */
    
    
    
    float lightDist = (dist_to_light / light_atten_end) - light_shadow_bias;
    
    if(lightDepth.r < lightDist) output.color.rgb = 0;
    else output.color.rgb = 1;
    
    //diffuse lighting
    float self_shadow = clamp(n_dot_l * 2.0, 0.0, 1.0);
  	 float3 direct_contrib = diffuse_color * n_dot_l;
    output.color.rgb *= self_shadow * direct_contrib;
    return output;
}
